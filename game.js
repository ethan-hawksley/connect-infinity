class ConnectFourGame {
  ...
  initializeEventListeners() {
    // Lobby controls
    document
      .getElementById('host-game-btn')
      .addEventListener('click', () => this.hostGame());
    document
      .getElementById('join-game-btn')
      .addEventListener('click', () => this.showJoinControls());
    document
    ...
    document
      ...
      .addEventListener('click', () => this.replayGame());
    document
    ...
    try {
      ...
      this.peer.on('open', (id) => {
        ...
        this.gameState.players = [
          { name: this.playerName, id: id, connected: true },
        ];
    ...
    for (let [dr, dc] of directions) {
      ...
      for (let i = 1; i < 4; i++) {
        ...
        if (
          ...
          c >= 0 &&
          c < 6 &&
          this.gameState.board[r][c] === playerId
        ) {
          count++;
        } else {
          break;
  ...
  updateGameInfo() {
    ...
    if (this.gameState.winner !== null) {
      ...
      const winners = this.gameState.players.filter(
        (_, index) => this.gameState.scores[index] === maxScore,
      );
      ...
      gameStatus.textContent =
        'Get 4 in a row to score! Game ends when board is full.';
